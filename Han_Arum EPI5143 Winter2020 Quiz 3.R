...
# (hint:  use the nrow() and ncol() and/or the dim()  R functions )
  ...
  library(tidyverse)
  view (mpg)
  nrow(mpg) #results from the console window: 234
  ncol(mpg) #results from the console window: 11
  dim(mpg) #results from the console window: 234 11 
  #ANSWER: Number of observations: 234; Number of variables: 11
...
# Question 2. modify and run the ggplot code to make each class of vehicle a different colour
  ...
  ggplot(data = mpg, aes(x = displ, y = hwy)) + geom_point() #formatted like the example from dataplot - gives a same plot
...
# Question 5. Modify the code to add a suitable title of your choice to your plot
  ...
  ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, 
  color = class, shape = drv, size = cyl)) +ggtitle("Highway fuel efficiency by engine size ")
  
  ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy, color = class, shape = drv, size = cyl))  
  + labs(title = "Highway fuel efficiency by engine size ")
